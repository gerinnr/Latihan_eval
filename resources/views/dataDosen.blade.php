...
<body class="bg-gradient-to-tr from-blue-50 via-white to-green-50 font-[system-ui] leading-normal tracking-normal">
  ...
  <div class="flex flex-col min-h-screen">
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <aside class="w-64 bg-white shadow-lg flex-shrink-0 border-r border-gray-300">
        <div class="p-6">
          ...
          <nav class="space-y-2">
            ...
            <a href="{{ route('dosen.index') }}" class="flex items-center gap-2 py-2 px-4 rounded-lg bg-blue-500 text-white font-medium shadow">
      ...
      <main class="flex-1 p-8 overflow-y-auto bg-white border-l border-gray-300">
        ...
        <!-- Page Header -->
        <div class="mb-6">
          <h2 class="text-3xl font-bold text-gray-800">📋 Data Dosen</h2>
          <p class="text-gray-500">Berikut adalah data dosen yang tersedia dalam sistem.</p>
        </div>

        <!-- Header Tabel -->
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-4">
          <div>
            <h3 class="text-xl font-semibold text-gray-800">Daftar Dosen</h3>
            <p class="text-sm text-gray-500">Kelola data dosen yang tersedia di sistem.</p>
          </div>

          <div class="flex flex-col md:flex-row items-center gap-2">
            <!-- Form Pencarian -->
            <form method="GET" action="{{ route('dosen.index') }}" class="flex items-center border rounded-md overflow-hidden bg-white shadow-sm">
            ...
            <!-- Tombol Tambah -->
            <a href="{{ route('dosen.create') }}" class="bg-green-500 text-white px-4 py-2 rounded-md text-sm hover:bg-green-700 shadow-sm">
          ...
          <table class="min-w-full table-auto text-sm">
            <thead>
              <tr class="bg-blue-200 text-gray-800 uppercase text-xs">
                ...
                <th class="py-3 px-4 text-center border">ID Dosen</th>
                <th class="py-3 px-4 border">Nama</th>
                <th class="py-3 px-4 border">NIDN</th>
                <th class="py-3 px-4 border">Email</th>
                <th class="py-3 px-4 border">Prodi</th>
            ...
            <tbody id="dosenTableBody">
              ...
              <tr class="hover:bg-blue-50 border-b">
                ...
                <td class="py-2 px-4 text-center">{{ $dsn['id'] }}</td>
                <td class="py-2 px-4">{{ $dsn['nama'] }}</td>
                <td class="py-2 px-4">{{ $dsn['nidn'] }}</td>
                <td class="py-2 px-4">{{ $dsn['email'] }}</td>
                <td class="py-2 px-4">{{ $dsn['prodi'] }}</td>
                <td class="py-2 px-4 text-center space-x-1">
                  <a href="{{ route('dosen.edit', $dsn['nidn']) }}" class="bg-yellow-400 text-white px-3 py-1 rounded hover:bg-yellow-500 transition">
                    <i class="fas fa-edit"></i>
                  </a>
                  <form action="{{ route('dosen.destroy', $dsn['nidn']) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
  ...
  <script>
    const rowsPerPage = 5;
    let currentPage = 1;
    const table = document.querySelector("#dosenTableBody");
